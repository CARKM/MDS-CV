...
class CoarseEncoder(nn.Module):
    def __init__(self, in_channels=3, depth=3, blocks=1, start_filters=32, residual=True, norm=nn.BatchNorm2d, act=F.relu):
        ...
        Args:
            in_channels (int, optional): input channels size. Defaults to 3.
            depth (int, optional): number of layers (CNN blocks). Defaults to 3.
            blocks (int, optional): the number of blocks for a depth. Defaults to 1.
            start_filters (int, optional): output dim channels. Defaults to 32. 
                                            if depth=1 --> 3 -> 32
                                            if depth=2 --> 3 -> 32 -> 64
                                            therefore, 3 -> 2**i * start_filters
            residual (bool, optional): residual connection. Defaults to True.
            norm (_type_, optional): batchnormal module. Defaults to nn.BatchNorm2d.
            act (_type_, optional): activation function. Defaults to F.relu.
        """
    ...
    def forward(self, x):
        ...
        return x, encoder_outs  # final output and the output of each layers

# shared the last two encoder and decoder here
