...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, blocks, residual=True,norm=nn.BatchNorm2d, 
        ...
        # concat 指的是 上下来源的合并 （resconnection）
        if self.concat:
...
class SelfAttentionSimple(nn.Module):
    def __init__(self, mode, in_channel, k_center=1, sim_metric='fc', project_mode='linear'):
        ...
        self.q_conv = conv1x1(in_channel, in_channel, stride=1, padding=0)
        self.k_conv = conv1x1(in_channel, in_channel*k_center, stride=1, padding=0)
        self.v_conv = conv1x1(in_channel, in_channel*k_center, stride=1, padding=0)
    ...
    def compute_attention(self, query, key, mask, eps=1):  # in: [B, C:128, 64, 64]
        ...
        # channels sum, in dim [b, c]
        # if the summation value less than the self.min_area set as self.min_area=100
        s_area = torch.clamp_min(
            torch.sum(importance_map, dim=[2,3]), self.min_area
        )[:,0:1]

        # key should be masked by the importance map
        if self.k_center != 2:
...
class SMRBlock(nn.Module):
    def __init__(self, args, ins, outs, norm=nn.BatchNorm2d,act=F.relu, blocks=1, residual=True, concat=True):
        ...
        self.primary_mask = nn.Sequential(*[
            conv1x1(in_channels=outs, out_channels=1, stride=1, padding=0),
            nn.Sigmoid()
        ])

        # attetion 得到进一步的mask
        self.self_calibrated = SelfAttentionSimple(
            self.mode, outs, sim_metric=args.sim_metric, 
            k_center=args.k_center, project_mode=args.project_mode
        )
